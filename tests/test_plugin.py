...
class WebCache(object):
    """Cache web data accessed by the plugin or PyMOL itself."""

    def __init__(self, pytestconfig, plugin_fetcher, pymol_fetcher):
        """Uses plugin_fetcher and pymol_fetcher to get data on cache miss."""
        self._pytestconfig = pytestconfig
        self._plugin_fetcher = plugin_fetcher
        self._pymol_fetcher = pymol_fetcher
        self._cache = {}
        self._num_accesses = 0
        self._num_hits = 0

    def access_plugin_url(self, url, description):
        """Interposer for plugin's get_data(url, description)"""
        return self._access('plugin', url, description)

    def access_pymol_url(self, finfo):
        """Interposer for pymol's file_read(finfo)"""
        if '://' in finfo:
            return self._access('pymol', finfo, 'pymol')
        else:
            # Don't cache local file accesses.
            return self._pymol_fetcher(finfo)

    def _access(self, source, url, description):
        ...
        # Cache miss - fetch data from web.
        if not self._pytestconfig.option.webcache_fetch:
            raise Exception(
                'Missing in webcache: %s (%s)\nFetching from web is disallowed.'
                % (description, url))
        if source == 'plugin':
            data = self._plugin_fetcher(url, description)
        elif source == 'pymol':
            data = self._pymol_fetcher(url)
        else:
            # Internal error in testing code.
            raise KeyError('No fetcher for source %s' % source)
        self._cache[key] = data
        return data

    def save_to_dir(self, path):
        """Saves cache data, one file per key, located in dir path."""
        # First purge all cache files in the cache dir.
        if not os.path.isdir(path):
            os.mkdir(path)
        print('Purging %d entries from webcache %s.' % (len(self._cache), path))
        num_purged = 0
        for filename in os.listdir(path):
            key = url_parse.unquote_plus(filename)
            if key.startswith(('plugin:', 'pymol:')):
                num_purged += 1
                os.unlink(os.path.join(path, filename))
        print('Purged %d files from webcache %s.' % (num_purged, path))
        # Then save the cache contents to the cache dir.
        print('Saving %d entries to webcache %s.' % (len(self._cache), path))
        for key, data in self._cache.items():
            filename = url_parse.quote_plus(key, safe='')  # must quote '/'
            if key.startswith('plugin:'):
                with open(os.path.join(path, filename), 'w') as file:
                    json.dump(data, file, indent=2, sort_keys=True)
            elif key.startswith('pymol:'):
                with open(os.path.join(path, filename), 'wb') as file:
                    file.write(data)
            else:
                # Internal error in testing code.
                raise KeyError("Can't save webcache key %s" % key)
    ...
    def load_from_dir(self, path):
        ...
        for filename in os.listdir(path):
            ...
            if key.startswith('plugin:'):
                ...
                pass  # Ignore all other files.
        print('Loaded %d entries from webcache %s.' % (len(self._cache), path))
...
def web_cache(pytestconfig):
    # --- setup ---
    if pytestconfig.option.webcache:
        plugin_fetcher = plugin.pdb._fetcher.get_data
        pymol_fetcher = pymol.cmd.file_read
        web_cache = WebCache(pytestconfig, plugin_fetcher, pymol_fetcher)
...
def cache_url_data_for_plugin(monkeypatch, web_cache):
    ...
    Wrap web cache around code so we don't hit the real PDB with repeated
    requests from these tests.

    Note that his fixture can't be session scoped like the cache itself since
    the monkeypatch fixture which it depends on is function scoped.
    """
    # --- setup ---
    if web_cache:
        monkeypatch.setattr(plugin.pdb._fetcher, 'get_data',
                            web_cache.access_plugin_url)
        monkeypatch.setattr(pymol.cmd, 'file_read', web_cache.access_pymol_url)
...
# ----- keep this test last -----
@pytest.mark.skipif(not web_cache, reason='web cache disabled')
